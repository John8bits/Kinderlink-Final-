...
if ($guardianEmail || $user) {
    $profileStmt = $conn->prepare("SELECT guardian_name, email FROM guardian WHERE email = ? OR guardian_name = ? LIMIT 1");
    $profileStmt->execute([$guardianEmail, $user]);
    $guardianProfile = $profileStmt->fetch(PDO::FETCH_ASSOC);
}
$profileName = $guardianProfile['guardian_name'] ?? $user;
$profileEmail = $guardianProfile['email'] ?? $guardianEmail;
...
$quarterOrder = array_keys($quarterDates);
...
$firstViewableQuarterIndex = max(0, (int) $currentQuarterIndex - 1);
$reportIsViewable = $selectedQuarterIndex !== false
    && $selectedQuarterIndex <= $currentQuarterIndex
    && $selectedQuarterIndex >= $firstViewableQuarterIndex;
$reportUnavailableMessage = '';
if (!$reportIsViewable) {
    if ($selectedQuarterIndex !== false && $selectedQuarterIndex > $currentQuarterIndex) {
        $reportUnavailableMessage = ($quarterDates[$selectedQuarter]['label'] ?? $selectedQuarter) . ' is not the current grading period yet.';
    } else {
        $reportUnavailableMessage = 'Only the current and previous grading periods are viewable.';
    }
}
...
<body>
                    ...
                    <div class="content-card">
                        ...
                        <?php else: ?>
                            <?php foreach ($attendanceHistory as $record): ?>
                                <?php
                                $statusClass = strtolower((string) ($record['status'] ?? ''));
                                $statusClass = in_array($statusClass, ['present', 'absent', 'late'], true) ? $statusClass : '';
                                ?>
                                <div class="list-row">
                                    <strong><?= htmlspecialchars(date('Y-m-d', strtotime($record['date']))) ?></strong>
                                    <span class="status-badge <?= e($statusClass) ?>"><?= htmlspecialchars(strtolower($record['status'])) ?></span>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
            ...
            <?php elseif ($page == 'reports'): ?>
                ...
                <?php else: ?>
                        ...
                        <select id="reportQuarterSelect" name="quarter" class="form-select" onchange="this.form.submit()">
                            <?php foreach ($quarterDates as $quarterKey => $quarter): ?>
                                ...
                                <option value="<?= e($quarterKey) ?>" <?= $selectedQuarter === $quarterKey ? 'selected' : '' ?> <?= $quarterViewable ? '' : 'disabled' ?>>
                                    <?= e($quarter['label'] . ' - ' . $quarterIndicator) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </form>
                    ...
                    <div class="content-card report-card">
                        <?php if (!$reportIsViewable): ?>
                            <div class="muted-empty">
                                <?= e($reportUnavailableMessage) ?>
                                <div class="report-subtitle">Selected Grading Period: <?= e($quarterDates[$selectedQuarter]['label'] ?? $selectedQuarter) ?> | Current Grading Period: <?= e($quarterDates[$currentQuarter]['label'] ?? $currentQuarter) ?></div>
                            </div>
                        <?php else: ?>
    ...
    <div id="editProfileModal" class="modal-overlay" style="display:none;">
        <div class="modal-box account-modal">
            ...
            <form method="POST" action="../Authentication/update_profile.php" class="profile-edit-form">
                ...
                <input id="profileName" class="form-input" type="text" name="name" value="<?= e($profileName) ?>" required>
                <div class="input-error" data-error-for="profileName"></div>

                <label for="profileEmail">Email</label>
                <input id="profileEmail" class="form-input" type="email" name="email" value="<?= e($profileEmail) ?>" required>
    ...
    <script>
        window.KinderLinkFlashMessages = [
            <?php if ($login_success): ?>{ icon: 'success', title: 'Guardian Login Successful!', text: <?= js('Welcome back, Guardian ' . $user) ?>, confirmButtonText: 'OK', confirmButtonColor: '#4361ee' },<?php endif; ?>
