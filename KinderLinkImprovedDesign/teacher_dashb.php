...
<body class="<?= e($page) ?>-page">
        ...
        <main>
            ...
            <?php elseif ($page == 'pupils'): ?>
                    ...
                    <form method="POST" action="../controllers/add_pupil.php" id="addPupilForm">
                        ...
                        <div class="form-grid">
                            ...
                            <div>
                                ...
                                <div class="input-error" id="pupilNameError"></div>
                            </div>
                        </div>
                        ...
                        <div class="form-grid">
                            ...
                            <div>
                                ...
                                <input class="form-input" type="text" name="home_address" placeholder="Enter address"
                                    required>
                            </div>
                        </div>
                        ...
                        <div class="form-grid-1">
                            ...
                            <textarea class="form-textarea" name="health_notes"
                ...
                <div class="form-card" id="editPupilForm" style="display:none;">
                    ...
                    <form method="POST" action="../controllers/update_pupil.php">
                    ...
                    <form method="GET" style="display: flex; gap: 15px; align-items: center;">
                        ...
                        <div style="display: flex; align-items: center; gap: 10px;">
                            ...
                            <input type="date" name="date" value="<?= e($date) ?>" min="<?= e($quarterRange['start']) ?>"
                                max="<?= e($quarterRange['end']) ?>" onchange="this.form.submit()"
            ...
            <?php elseif ($page == 'milestones'): ?>
                ...
                <?php else: ?>
                    ...
                    <form method="POST" action="../controllers/save_pupil_milestones.php">
                        ...
                        <input type="hidden" name="quarter" value="<?= e($selectedQuarter) ?>">
                        <div class="milestone-checklist-card <?= $selectedPupilAbsentToday ? 'is-locked' : '' ?>">
                            ...
                            <?php else: ?>
                                <?php foreach ($pupilMilestones as $item): ?>
                                    <label
                                        class="checklist-item <?= $item['status'] === 'Completed' ? 'completed' : '' ?> <?= $selectedPupilAbsentToday ? 'disabled' : '' ?>">
                                        <div class="checklist-input">
                                            <input type="checkbox" name="completed[]" value="<?= (int) $item['milestone_id'] ?>"
                                                <?= $item['status'] === 'Completed' ? 'checked' : '' ?>                 <?= $selectedPupilAbsentToday ? 'disabled' : '' ?>>
                                        </div>
                        ...
                        <div class="milestone-actions-row">
                            <button type="submit" class="form-btn btn-save" id="saveBtn" <?= $selectedPupilAbsentToday ? 'disabled' : '' ?>>
    ...
    <script>
        ...
        function viewFullReport(reportData) {
            const report = reportData || window.currentFullReport;
            if (!report) {
            ...
            const attendance = report.attendance || { present: 0, absent: 0, late: 0 };
            const milestones = Array.isArray(report.milestones) ? report.milestones : [];
            const totalAttendance = Number(attendance.present || 0) + Number(attendance.absent || 0) + Number(attendance.late || 0);
            const completedMilestones = milestones.filter((item) => item.status === 'Completed').length;
            const milestoneRows = milestones.length ? milestones.map((item) => `
                <tr>
            ...
            reportWindow.document.write(`
                ...
                <h2>Attendance Summary</h2><div class="summary-grid"><div class="box"><div class="label">Present</div><div class="value">${Number(attendance.present || 0)}</div></div><div class="box"><div class="label">Absent</div><div class="value">${Number(attendance.absent || 0)}</div></div><div class="box"><div class="label">Late</div><div class="value">${Number(attendance.late || 0)}</div></div><div class="box"><div class="label">Total Records</div><div class="value">${totalAttendance}</div></div></div>
                <h2>Milestone Progress</h2><div class="box"><strong>${completedMilestones} of ${milestones.length} milestones completed</strong></div><table><thead><tr><th>Milestone</th><th>Status</th><th>Date Completed</th></tr></thead><tbody>${milestoneRows}</tbody></table></div></body></html>
            `);
